...
import com.kevinguegancamillepaviot.pokemon.provider.TypeDePokemonsProviderAdapter;
import com.kevinguegancamillepaviot.pokemon.provider.TypeDePokemonstoPokedexsProviderAdapter;
...
public abstract class TypeDePokemonsProviderUtilsBase
    ...
    public Uri insert(final TypeDePokemons item) {
        ...
        if (item.getPokedex() != null && item.getPokedex().size() > 0) {
            for (Pokedexs pokedexs : item.getPokedex()) {
                ContentValues pokedexsValues = new ContentValues();
                pokedexsValues.put(
                        TypeDePokemonstoPokedexsContract.COL_POKEDEX_ID,
                        pokedexs.getId());

                operations.add(ContentProviderOperation.newInsert(
                    TypeDePokemonstoPokedexsProviderAdapter.TYPEDEPOKEMONSTOPOKEDEXS_URI)
                        .withValues(pokedexsValues)
                        .withValueBackReference(
    ...
    public int update(final TypeDePokemons item) {
        ...
        operations.add(ContentProviderOperation.newDelete(TypeDePokemonstoPokedexsProviderAdapter.TYPEDEPOKEMONSTOPOKEDEXS_URI)
                .withSelection(TypeDePokemonstoPokedexsContract.COL_TYPEDEPOKEMONSINTERNALID_ID + "= ?",
                                new String[]{String.valueOf(item.getId())})
                .build());

        for (Pokedexs pokedexs : item.getPokedex()) {
            ContentValues pokedexsValues = new ContentValues();
            pokedexsValues.put(TypeDePokemonstoPokedexsContract.COL_POKEDEX_ID,
                    pokedexs.getId());
            pokedexsValues.put(TypeDePokemonstoPokedexsContract.COL_TYPEDEPOKEMONSINTERNALID_ID,
                    item.getId());

            operations.add(ContentProviderOperation.newInsert(TypeDePokemonstoPokedexsProviderAdapter.TYPEDEPOKEMONSTOPOKEDEXS_URI)
                    .withValues(pokedexsValues)
                    .build());
        }
     ...
     * @return Pokedexs
     */
    public ArrayList<Pokedexs> getAssociatePokedex(
            final TypeDePokemons item) {
        ArrayList<Pokedexs> result;
        ContentResolver prov = this.getContext().getContentResolver();
        android.database.Cursor typeDePokemonstoPokedexsCursor = prov.query(
                TypeDePokemonstoPokedexsProviderAdapter.TYPEDEPOKEMONSTOPOKEDEXS_URI,
        ...
        if (typeDePokemonstoPokedexsCursor.getCount() > 0) {
            CriteriaExpression pokedexsCrits =
                    new CriteriaExpression(GroupType.AND);
            Criterion inCrit = new Criterion();
            ArrayValue arrayValue = new ArrayValue();
            inCrit.setKey(PokedexsContract.ALIASED_COL_ID);
            inCrit.setType(Type.IN);
            inCrit.addValue(arrayValue);
            pokedexsCrits.add(inCrit);

            while (typeDePokemonstoPokedexsCursor.moveToNext()) {
                int index = typeDePokemonstoPokedexsCursor.getColumnIndex(
                        TypeDePokemonstoPokedexsContract.COL_POKEDEX_ID);
                String pokedexsId = typeDePokemonstoPokedexsCursor.getString(index);

                arrayValue.addValue(pokedexsId);
            }
            typeDePokemonstoPokedexsCursor.close();
            android.database.Cursor pokedexsCursor = prov.query(
                    PokedexsProviderAdapter.POKEDEXS_URI,
                    PokedexsContract.ALIASED_COLS,
                    pokedexsCrits.toSQLiteSelection(),
                    pokedexsCrits.toSQLiteSelectionArgs(),
                    null);

            result = PokedexsContract.cursorToItems(pokedexsCursor);
            pokedexsCursor.close();
        } else {
            result = new ArrayList<Pokedexs>();
        }
