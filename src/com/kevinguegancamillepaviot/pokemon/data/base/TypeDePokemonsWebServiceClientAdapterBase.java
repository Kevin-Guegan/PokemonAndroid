...
public abstract class TypeDePokemonsWebServiceClientAdapterBase
    ...
    /**
     * Get the TypeDePokemons associated with a Pokedexs. Uses the route : pokedexs/%Pokedexs_id%/typedepokemons.
     * @param typeDePokemons : The TypeDePokemons that will be returned
     * @param pokedexs : The associated pokedexs
     * @return -1 if an error has occurred. 0 if not.
     */
    public int getByPokedexs(TypeDePokemons typeDePokemons, Pokedexs pokedexs) {
        int result = -1;
        String response = this.invokeRequest(
                    Verb.GET,
                    String.format(
     ...
     * @return The number of TypeDePokemonss returned
     */
    public int getByTypestypeDePokemonInternal(List<TypeDePokemons> typeDePokemonss, Types types) {
        int result = -1;
        String response = this.invokeRequest(
                    Verb.GET,
                    String.format(
            ...
            try {
                ...
                if (json.has(TypeDePokemonsWebServiceClientAdapter.JSON_POKEDEX)
                    ...
                    try {
                        ...
                        if (pokedexAdapter.extract(
                                json.optJSONObject(
                                        TypeDePokemonsWebServiceClientAdapter.JSON_POKEDEX),
                                        pokedex)) {
                            typeDePokemons.setPokedex(pokedex);
                        }
                    } catch (Exception e) {
                        Log.e(TAG, "Json doesn't contains Pokedexs data");
                    }
                }
            } catch (JSONException e) {
                Log.e(TAG, e.getMessage());
    ...
    public boolean extractCursor(JSONObject json, MatrixCursor cursor) {
        ...
        if (id != null) {
            try {
                ...
                if (json.has(TypeDePokemonsWebServiceClientAdapter.JSON_POKEDEX)) {
                    JSONObject pokedexJson = json.getJSONObject(
                            TypeDePokemonsWebServiceClientAdapter.JSON_POKEDEX);
                    row[9] = pokedexJson.getString(
                            PokedexsWebServiceClientAdapter.JSON_ID);
                }
        ...
        try {
            ...
            if (typeDePokemons.getPokedex() != null) {
                ...
                params.put(TypeDePokemonsWebServiceClientAdapter.JSON_POKEDEX,
                        pokedexAdapter.itemIdToJson(typeDePokemons.getPokedex()));
            }
        } catch (JSONException e) {
            Log.e(TAG, e.getMessage());
        }
    ...
    public JSONObject contentValuesToJson(ContentValues values) {
        ...
        try {
            ...
            params.put(TypeDePokemonsWebServiceClientAdapter.JSON_POKEDEX,
                    pokedexAdapter.contentValuesToJson(values));
        } catch (JSONException e) {
            Log.e(TAG, e.getMessage());
        }
