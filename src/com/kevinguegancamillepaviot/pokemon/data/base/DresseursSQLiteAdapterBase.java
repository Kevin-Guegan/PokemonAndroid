...
public abstract class DresseursSQLiteAdapterBase
    ...
    public static String getSchema() {
        ...
        + DresseursContract.TABLE_NAME    + " ("
         ...
         + DresseursContract.COL_LOGIN    + " VARCHAR NOT NULL,"
         + DresseursContract.COL_PASSWORD    + " VARCHAR NOT NULL"
...
;
    ...
    public Dresseurs getByID(final int id) {
        ...
        final NpcsSQLiteAdapter npcAdapter =
                new NpcsSQLiteAdapter(this.ctx);
        npcAdapter.open(this.mDatabase);
        android.database.Cursor npcCursor = npcAdapter
                    .getByDresseursnpcInternal(
                            result.getId(),
                            NpcsContract.ALIASED_COLS,
                            null,
                            null,
                            null);
        result.setNpc(npcAdapter.cursorToItems(npcCursor));
    ...
    public long insert(final Dresseurs item) {
        ...
        if (item.getNpc() != null) {
            NpcsSQLiteAdapterBase npcAdapter =
                    new NpcsSQLiteAdapter(this.ctx);
            npcAdapter.open(this.mDatabase);
            for (Npcs npcs
                        : item.getNpc()) {
                npcAdapter.insertOrUpdateWithDresseursNpc(
                                    npcs,
                                    insertResult);
            }
        }
