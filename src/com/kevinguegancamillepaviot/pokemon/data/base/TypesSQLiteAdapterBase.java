...
;
    ...
    public Types getByID(final int id) {
        ...
        final TypeDePokemonsSQLiteAdapter typeDePokemonAdapter =
                new TypeDePokemonsSQLiteAdapter(this.ctx);
        typeDePokemonAdapter.open(this.mDatabase);
        android.database.Cursor typedepokemonCursor = typeDePokemonAdapter
                    .getByTypestypeDePokemonInternal(
                            result.getId(),
                            TypeDePokemonsContract.ALIASED_COLS,
                            null,
                            null,
                            null);
        result.setTypeDePokemon(typeDePokemonAdapter.cursorToItems(typedepokemonCursor));

        typedepokemonCursor.close();
        final AttaquesSQLiteAdapter attaqueAdapter =
                new AttaquesSQLiteAdapter(this.ctx);
        attaqueAdapter.open(this.mDatabase);
        android.database.Cursor attaqueCursor = attaqueAdapter
                    .getByTypesattaqueInternal(
                            result.getId(),
                            AttaquesContract.ALIASED_COLS,
                            null,
                            null,
                            null);
        result.setAttaque(attaqueAdapter.cursorToItems(attaqueCursor));
    ...
    public long insert(final Types item) {
        ...
        if (item.getTypeDePokemon() != null) {
            TypeDePokemonsSQLiteAdapterBase typeDePokemonAdapter =
                    new TypeDePokemonsSQLiteAdapter(this.ctx);
            typeDePokemonAdapter.open(this.mDatabase);
            for (TypeDePokemons typedepokemons
                        : item.getTypeDePokemon()) {
                typeDePokemonAdapter.insertOrUpdateWithTypesTypeDePokemon(
                                    typedepokemons,
                                    insertResult);
            }
        }
        if (item.getAttaque() != null) {
            AttaquesSQLiteAdapterBase attaqueAdapter =
                    new AttaquesSQLiteAdapter(this.ctx);
            attaqueAdapter.open(this.mDatabase);
            for (Attaques attaques
                        : item.getAttaque()) {
                attaqueAdapter.insertOrUpdateWithTypesAttaque(
                                    attaques,
                                    insertResult);
            }
        }
