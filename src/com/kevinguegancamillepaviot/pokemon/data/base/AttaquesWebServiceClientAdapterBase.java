...
public abstract class AttaquesWebServiceClientAdapterBase
    ...
    /**
     * Get the Attaquess associated with a Types. Uses the route : types/%Types_id%/attaques.
     * @param attaquess : The list in which the Attaquess will be returned
     * @param types : The associated types
     * @return The number of Attaquess returned
     */
    public int getByType(List<Attaques> attaquess, Types types) {
        int result = -1;
        String response = this.invokeRequest(
                    Verb.GET,
                    String.format(
        ...
        if (this.isValidResponse(response) && this.isValidRequest()) {
            try {
                JSONObject json = new JSONObject(response);
                result = this.extractItems(json, "Attaquess", attaquess);
            } catch (JSONException e) {
                Log.e(TAG, e.getMessage());
                attaquess = null;
            }
     ...
     * @return The number of Attaquess returned
     */
    public int getByTypesattaqueInternal(List<Attaques> attaquess, Types types) {
        int result = -1;
        String response = this.invokeRequest(
                    Verb.GET,
                    String.format(
