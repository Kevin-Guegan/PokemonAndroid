...
public abstract class AttaquesSQLiteAdapterBase
    ...
    public static String getSchema() {
        ...
        + AttaquesContract.TABLE_NAME    + " ("
         ...
         + AttaquesContract.COL_TYPE_ID    + " INTEGER,"
         + AttaquesContract.COL_TYPESATTAQUEINTERNAL_ID    + " INTEGER,"

        
         + "FOREIGN KEY(" + AttaquesContract.COL_TYPE_ID + ") REFERENCES " 
             + TypesContract.TABLE_NAME 
                + " (" + TypesContract.COL_ID + "),"
         + "FOREIGN KEY(" + AttaquesContract.COL_TYPESATTAQUEINTERNAL_ID + ") REFERENCES " 
             + TypesContract.TABLE_NAME 
                + " (" + TypesContract.COL_ID + ")"
        + ");"
;
    ...
    /**
     * Find & read Attaques by TypesattaqueInternal.
     * @param typesattaqueinternalId typesattaqueinternalId
     * @param orderBy Order by string (can be null)
     * @return List of Attaques entities
     */
     public android.database.Cursor getByTypesattaqueInternal(final int typesattaqueinternalId, String[] projection, String selection, String[] selectionArgs, String orderBy) {
        String idSelection = AttaquesContract.COL_TYPESATTAQUEINTERNAL_ID + "= ?";
        String idSelectionArgs = String.valueOf(typesattaqueinternalId);
        if (!Strings.isNullOrEmpty(selection)) {
            selection += " AND " + idSelection;
            selectionArgs = ObjectArrays.concat(selectionArgs, idSelectionArgs);
        } else {
    ...
    public long insert(final Attaques item) {
        ...
        final ContentValues values =
                AttaquesContract.itemToContentValues(item, 0);
     ...
     * @return count of updated entities
     */
    public int updateWithTypesAttaque(
                    Attaques item,
                    int typesattaqueInternalId) {
        if (PokemonApplication.DEBUG) {
            android.util.Log.d(TAG, "Update DB(" + AttaquesContract.TABLE_NAME + ")");
        }
     ...
     * @return 1 if everything went well, 0 otherwise
     */
    public int insertOrUpdateWithTypesAttaque(
            Attaques item, int typesId) {
        int result = 0;
        if (this.getByID(item.getId()) != null) {
            // Item already exists => update it
            result = this.updateWithTypesAttaque(item,
                    typesId);
        } else {
            // Item doesn't exist => create it
            long id = this.insertWithTypesAttaque(item,
                    typesId);
            if (id != 0) {
                result = 1;
            }
     ...
     * @return Id of the Attaques entity
     */
    public long insertWithTypesAttaque(
            Attaques item, int typesId) {
        if (PokemonApplication.DEBUG) {
            android.util.Log.d(TAG, "Insert DB(" + AttaquesContract.TABLE_NAME + ")");
        }

        ContentValues values = AttaquesContract.itemToContentValues(item,
                typesId);
        values.remove(AttaquesContract.COL_ID);
