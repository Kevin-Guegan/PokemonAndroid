...
public abstract class ObjetsWebServiceClientAdapterBase
    ...
    /**
     * Get the Objets associated with a TypeObjets. Uses the route : typeobjets/%TypeObjets_id%/objets.
     * @param objets : The Objets that will be returned
     * @param typeobjets : The associated typeobjets
     * @return -1 if an error has occurred. 0 if not.
     */
    public int getByTypeObjets(Objets objets, TypeObjets typeObjets) {
        int result = -1;
        String response = this.invokeRequest(
                    Verb.GET,
                    String.format(
    ...
    public boolean extract(JSONObject json, Objets objets) {
        ...
        if (result) {
            try {
                ...
                if (json.has(ObjetsWebServiceClientAdapter.JSON_OBJET)
                        && !json.isNull(ObjetsWebServiceClientAdapter.JSON_OBJET)) {

                    try {
                        TypeObjetsWebServiceClientAdapter objetAdapter =
                                new TypeObjetsWebServiceClientAdapter(this.context);
                        TypeObjets objet =
                                new TypeObjets();

                        if (objetAdapter.extract(
                                json.optJSONObject(
                                        ObjetsWebServiceClientAdapter.JSON_OBJET),
                                        objet)) {
                            objets.setObjet(objet);
                        }
                    } catch (Exception e) {
                        Log.e(TAG, "Json doesn't contains TypeObjets data");
                    }
                }
            } catch (JSONException e) {
                Log.e(TAG, e.getMessage());
    ...
    public boolean extractCursor(JSONObject json, MatrixCursor cursor) {
        ...
        if (id != null) {
            try {
                ...
                if (json.has(ObjetsWebServiceClientAdapter.JSON_OBJET)) {
                    JSONObject objetJson = json.getJSONObject(
                            ObjetsWebServiceClientAdapter.JSON_OBJET);
                    row[3] = objetJson.getString(
                            TypeObjetsWebServiceClientAdapter.JSON_ID);
                }
    ...
    public JSONObject itemToJson(Objets objets) {
        ...
        try {
            ...
            if (objets.getObjet() != null) {
                TypeObjetsWebServiceClientAdapter objetAdapter =
                        new TypeObjetsWebServiceClientAdapter(this.context);

                params.put(ObjetsWebServiceClientAdapter.JSON_OBJET,
                        objetAdapter.itemIdToJson(objets.getObjet()));
            }
        } catch (JSONException e) {
            Log.e(TAG, e.getMessage());
        }
    ...
    public JSONObject contentValuesToJson(ContentValues values) {
        ...
        try {
            ...
            params.put(ObjetsWebServiceClientAdapter.JSON_OBJET,
                    objetAdapter.contentValuesToJson(values));
        } catch (JSONException e) {
            Log.e(TAG, e.getMessage());
        }
