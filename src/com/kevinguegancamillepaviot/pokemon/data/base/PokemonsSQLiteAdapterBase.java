...
import com.kevinguegancamillepaviot.pokemon.harmony.util.DateUtils;
...
public abstract class PokemonsSQLiteAdapterBase
    ...
    public static String getSchema() {
        ...
        + PokemonsContract.TABLE_NAME    + " ("
         ...
         + PokemonsContract.COL_NPCSPOKEMONINTERNAL_ID    + " INTEGER,"
         ...
         + "FOREIGN KEY(" + PokemonsContract.COL_NPCSPOKEMONINTERNAL_ID + ") REFERENCES " 
...
;
    ...
    /**
     * Find & read Pokemons by NpcspokemonInternal.
     * @param npcspokemoninternalId npcspokemoninternalId
     * @param orderBy Order by string (can be null)
     * @return List of Pokemons entities
     */
     public android.database.Cursor getByNpcspokemonInternal(final int npcspokemoninternalId, String[] projection, String selection, String[] selectionArgs, String orderBy) {
        String idSelection = PokemonsContract.COL_NPCSPOKEMONINTERNAL_ID + "= ?";
        String idSelectionArgs = String.valueOf(npcspokemoninternalId);
     ...
     *
     * @param item The Pokemons entity to persist
     * @param npcsId The npcs id
     * @return count of updated entities
     */
    public int updateWithNpcsPokemon(
                    Pokemons item,
                    int npcspokemonInternalId) {
        if (PokemonApplication.DEBUG) {
            android.util.Log.d(TAG, "Update DB(" + PokemonsContract.TABLE_NAME + ")");
        }
     ...
     * @return 1 if everything went well, 0 otherwise
     */
    public int insertOrUpdateWithNpcsPokemon(
            Pokemons item, int npcsId) {
        int result = 0;
        if (this.getByID(item.getId()) != null) {
            // Item already exists => update it
            result = this.updateWithNpcsPokemon(item,
                    npcsId);
        } else {
            // Item doesn't exist => create it
            long id = this.insertWithNpcsPokemon(item,
                    npcsId);
            if (id != 0) {
                result = 1;
            }
     ...
     * @return Id of the Pokemons entity
     */
    public long insertWithNpcsPokemon(
            Pokemons item, int npcsId) {
        if (PokemonApplication.DEBUG) {
            android.util.Log.d(TAG, "Insert DB(" + PokemonsContract.TABLE_NAME + ")");
        }
