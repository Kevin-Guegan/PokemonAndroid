/**************************************************************************
 * TypeDePokemonsListActivity.java, pokemon Android
 *
 * Copyright 2016
 * Description : 
 * Author(s)   : Harmony
...
/**
 * This class will display TypeDePokemons entities in a list.
 *
 * This activity has a different behaviour wether you're on a tablet or a phone.
 * - On a phone, you will have a TypeDePokemons list. A click on an item will get 
 * you to TypeDePokemonsShowActivity.
 * - On a tablet, the TypeDePokemonsShowFragment will be displayed right next to
 * the list.
 */
public class TypeDePokemonsListActivity 
        extends HarmonyFragmentActivity
        implements HarmonyListFragment.OnClickCallback,
                HarmonyListFragment.OnLoadCallback {

    /** Associated list fragment. */
    protected TypeDePokemonsListFragment listFragment;
    /** Associated detail fragment if any (in case of tablet). */
    protected TypeDePokemonsShowFragment detailFragment;
    ...
    protected void onPostCreate(Bundle savedInstanceState) {
        ...
        this.detailFragment = (TypeDePokemonsShowFragment) 
                        this.getSupportFragmentManager().findFragmentById(
                                R.id.fragment_show);
        this.listFragment = (TypeDePokemonsListFragment)
                        this.getSupportFragmentManager().findFragmentById(
                                R.id.fragment_list);
    ...
    protected int getContentView() {
        return R.layout.activity_typedepokemons_list;
    }
    ...
    public void onListItemClick(ListView l, View v, int position, long id) {
        ...
        if (this.isDualMode()) {
            ...
            final Intent intent = new Intent(this, TypeDePokemonsShowActivity.class);
            final TypeDePokemons item = TypeDePokemonsContract
                    .cursorToItem((android.database.Cursor)
                            l.getItemAtPosition(position));
            Bundle extras = new Bundle();
            extras.putParcelable(TypeDePokemonsContract.PARCEL, item);
     ...
     */
    private void loadDetailFragment(TypeDePokemons item) {
    ...
    private void selectListItem(int listPosition) {
        ...
        if (listSize > 0) {
            ...
            TypeDePokemons item = TypeDePokemonsContract.cursorToItem(
