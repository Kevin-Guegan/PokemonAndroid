/**************************************************************************
 * ObjetsListActivity.java, pokemon Android
 *
 * Copyright 2016
 * Description : 
 * Author(s)   : Harmony
...
/**
 * This class will display Objets entities in a list.
 *
 * This activity has a different behaviour wether you're on a tablet or a phone.
 * - On a phone, you will have a Objets list. A click on an item will get 
 * you to ObjetsShowActivity.
 * - On a tablet, the ObjetsShowFragment will be displayed right next to
 * the list.
 */
public class ObjetsListActivity 
        extends HarmonyFragmentActivity
        implements HarmonyListFragment.OnClickCallback,
                HarmonyListFragment.OnLoadCallback {

    /** Associated list fragment. */
    protected ObjetsListFragment listFragment;
    /** Associated detail fragment if any (in case of tablet). */
    protected ObjetsShowFragment detailFragment;
    ...
    protected void onPostCreate(Bundle savedInstanceState) {
        ...
        this.detailFragment = (ObjetsShowFragment) 
                        this.getSupportFragmentManager().findFragmentById(
                                R.id.fragment_show);
        this.listFragment = (ObjetsListFragment)
                        this.getSupportFragmentManager().findFragmentById(
                                R.id.fragment_list);
    ...
    protected int getContentView() {
        return R.layout.activity_objets_list;
    }
    ...
    public void onListItemClick(ListView l, View v, int position, long id) {
        ...
        if (this.isDualMode()) {
            ...
            final Intent intent = new Intent(this, ObjetsShowActivity.class);
            final Objets item = ObjetsContract
                    .cursorToItem((android.database.Cursor)
                            l.getItemAtPosition(position));
            Bundle extras = new Bundle();
            extras.putParcelable(ObjetsContract.PARCEL, item);
     ...
     */
    private void loadDetailFragment(Objets item) {
    ...
    private void selectListItem(int listPosition) {
        ...
        if (listSize > 0) {
            ...
            Objets item = ObjetsContract.cursorToItem(
