/**************************************************************************
 * PokemonsListActivity.java, pokemon Android
 *
 * Copyright 2016
 * Description : 
 * Author(s)   : Harmony
...
/**
 * This class will display Pokemons entities in a list.
 *
 * This activity has a different behaviour wether you're on a tablet or a phone.
 * - On a phone, you will have a Pokemons list. A click on an item will get 
 * you to PokemonsShowActivity.
 * - On a tablet, the PokemonsShowFragment will be displayed right next to
 * the list.
 */
public class PokemonsListActivity 
        extends HarmonyFragmentActivity
        implements HarmonyListFragment.OnClickCallback,
                HarmonyListFragment.OnLoadCallback {

    /** Associated list fragment. */
    protected PokemonsListFragment listFragment;
    /** Associated detail fragment if any (in case of tablet). */
    protected PokemonsShowFragment detailFragment;
    ...
    protected void onPostCreate(Bundle savedInstanceState) {
        ...
        this.detailFragment = (PokemonsShowFragment) 
                        this.getSupportFragmentManager().findFragmentById(
                                R.id.fragment_show);
        this.listFragment = (PokemonsListFragment)
                        this.getSupportFragmentManager().findFragmentById(
                                R.id.fragment_list);
    ...
    protected int getContentView() {
        return R.layout.activity_pokemons_list;
    }
    ...
    public void onListItemClick(ListView l, View v, int position, long id) {
        ...
        if (this.isDualMode()) {
            ...
            final Intent intent = new Intent(this, PokemonsShowActivity.class);
            final Pokemons item = PokemonsContract
                    .cursorToItem((android.database.Cursor)
                            l.getItemAtPosition(position));
            Bundle extras = new Bundle();
            extras.putParcelable(PokemonsContract.PARCEL, item);
     ...
     */
    private void loadDetailFragment(Pokemons item) {
    ...
    private void selectListItem(int listPosition) {
        ...
        if (listSize > 0) {
            ...
            Pokemons item = PokemonsContract.cursorToItem(
