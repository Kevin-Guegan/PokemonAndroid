/**************************************************************************
 * AttaquesListActivity.java, pokemon Android
 *
 * Copyright 2016
 * Description : 
 * Author(s)   : Harmony
...
/**
 * This class will display Attaques entities in a list.
 *
 * This activity has a different behaviour wether you're on a tablet or a phone.
 * - On a phone, you will have a Attaques list. A click on an item will get 
 * you to AttaquesShowActivity.
 * - On a tablet, the AttaquesShowFragment will be displayed right next to
 * the list.
 */
public class AttaquesListActivity 
        extends HarmonyFragmentActivity
        implements HarmonyListFragment.OnClickCallback,
                HarmonyListFragment.OnLoadCallback {

    /** Associated list fragment. */
    protected AttaquesListFragment listFragment;
    /** Associated detail fragment if any (in case of tablet). */
    protected AttaquesShowFragment detailFragment;
    ...
    protected void onPostCreate(Bundle savedInstanceState) {
        ...
        this.detailFragment = (AttaquesShowFragment) 
                        this.getSupportFragmentManager().findFragmentById(
                                R.id.fragment_show);
        this.listFragment = (AttaquesListFragment)
                        this.getSupportFragmentManager().findFragmentById(
                                R.id.fragment_list);
    ...
    protected int getContentView() {
        return R.layout.activity_attaques_list;
    }
    ...
    public void onListItemClick(ListView l, View v, int position, long id) {
        ...
        if (this.isDualMode()) {
            ...
            final Intent intent = new Intent(this, AttaquesShowActivity.class);
            final Attaques item = AttaquesContract
                    .cursorToItem((android.database.Cursor)
                            l.getItemAtPosition(position));
            Bundle extras = new Bundle();
            extras.putParcelable(AttaquesContract.PARCEL, item);
     ...
     */
    private void loadDetailFragment(Attaques item) {
    ...
    private void selectListItem(int listPosition) {
        ...
        if (listSize > 0) {
            ...
            Attaques item = AttaquesContract.cursorToItem(
