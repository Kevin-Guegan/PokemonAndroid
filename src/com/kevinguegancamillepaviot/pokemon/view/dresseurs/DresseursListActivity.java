/**************************************************************************
 * DresseursListActivity.java, pokemon Android
 *
 * Copyright 2016
 * Description : 
 * Author(s)   : Harmony
...
/**
 * This class will display Dresseurs entities in a list.
 *
 * This activity has a different behaviour wether you're on a tablet or a phone.
 * - On a phone, you will have a Dresseurs list. A click on an item will get 
 * you to DresseursShowActivity.
 * - On a tablet, the DresseursShowFragment will be displayed right next to
 * the list.
 */
public class DresseursListActivity 
        extends HarmonyFragmentActivity
        implements HarmonyListFragment.OnClickCallback,
                HarmonyListFragment.OnLoadCallback {

    /** Associated list fragment. */
    protected DresseursListFragment listFragment;
    /** Associated detail fragment if any (in case of tablet). */
    protected DresseursShowFragment detailFragment;
    ...
    protected void onPostCreate(Bundle savedInstanceState) {
        ...
        this.detailFragment = (DresseursShowFragment) 
                        this.getSupportFragmentManager().findFragmentById(
                                R.id.fragment_show);
        this.listFragment = (DresseursListFragment)
                        this.getSupportFragmentManager().findFragmentById(
                                R.id.fragment_list);
    ...
    protected int getContentView() {
        return R.layout.activity_dresseurs_list;
    }
    ...
    public void onListItemClick(ListView l, View v, int position, long id) {
        ...
        if (this.isDualMode()) {
            ...
            final Intent intent = new Intent(this, DresseursShowActivity.class);
            final Dresseurs item = DresseursContract
                    .cursorToItem((android.database.Cursor)
                            l.getItemAtPosition(position));
            Bundle extras = new Bundle();
            extras.putParcelable(DresseursContract.PARCEL, item);
     ...
     */
    private void loadDetailFragment(Dresseurs item) {
    ...
    private void selectListItem(int listPosition) {
        ...
        if (listSize > 0) {
            ...
            Dresseurs item = DresseursContract.cursorToItem(
