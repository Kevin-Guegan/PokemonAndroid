...
public abstract class TypeDePokemonsUtilsBase {
    ...
    public static TypeDePokemons generateRandom(android.content.Context ctx){
        ...
        ArrayList<Pokedexs> relatedPokedexs = new ArrayList<Pokedexs>();
        relatedPokedexs.add(PokedexsUtils.generateRandom(ctx));
        typeDePokemons.setPokedex(relatedPokedexs);
    ...
    public static boolean equals(TypeDePokemons typeDePokemons1,
        ...
        if (typeDePokemons1!=null && typeDePokemons2 !=null){
            ...
            if (typeDePokemons1.getPokedex() != null
                ...
                Assert.assertEquals(typeDePokemons1.getPokedex().size(),
                    typeDePokemons2.getPokedex().size());
                if (checkRecursiveId) {
                    for (Pokedexs pokedex1 : typeDePokemons1.getPokedex()) {
                        boolean found = false;
                        for (Pokedexs pokedex2 : typeDePokemons2.getPokedex()) {
                            if (pokedex1.getId() == pokedex2.getId()) {
                                found = true;
                            }
                        }
                        Assert.assertTrue(
                                String.format(
                                        "Couldn't find associated pokedex (id = %s) in TypeDePokemons (id = %s)",
                                        pokedex1.getId(),
                                        typeDePokemons1.getId()),
                                found);
                    }
